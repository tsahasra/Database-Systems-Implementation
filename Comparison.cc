...
int Comparison::PerformEqualityCheck(int att) {
	if (op == Equals) {
		if (whichAtt1 == att) {
			return whichAtt2;
		} else {
...
void OrderMaker :: GetAtts(int* wa){
	for (int i=0;i< numAtts;++i){
		wa[i] = whichAtts[i];
	}
}

void OrderMaker :: GetTypes(Type* wt){
	for (int i=0;i< numAtts;++i){
		wt[i] = whichTypes[i];
	}
}
...
// set attributes
void OrderMaker :: SetAtts(int* wa){
	for(int i=0;i<numAtts;++i){
		whichAtts[i] = wa[i];
	}
}
...
// set types
void OrderMaker :: SetTypes(Type* wt){
	for(int i=0;i<numAtts;++i){
		whichTypes[i] = wt[i];
	}
}
...
void OrderMaker :: SetNumOfAtts(int natts){
	numAtts = natts;
}
...
int CNF :: HasEqualityCheck(int attribute){
	int literal;
	for (int i = 0; i < numAnds; i++) {
		if (orLens[i] == 1 && (literal = orList[i][0].PerformEqualityCheck(attribute)) != -1) {
			return literal;
		}
	}
	return -1;
