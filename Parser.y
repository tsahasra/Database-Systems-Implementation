...
%{
	...
	struct InsertOperation *insertOp; // insert operation parameters
	struct CreateAttList *createAtts; // attributes for the new table being created
	struct SortAttList *sortAtts; // attributes to be sorted on
	char *ctablename; // create table parameter
	char *dtablename; // drop table parameter
	char *outfilename; // file to which query output will be written to
	int cfiletype; // create file type
	int outtype; // set output type
...
%union {
	...
	struct InsertOperation *myInsertOperation;
	struct CreateAttList *myCreateAttList;
	struct SortAttList *mySortAttList;
	char *actualChars;
	char whichOne;
}
...
%type <myNames> Atts
%type <myInsertOperation> Inparams
%type <myCreateAttList> CreateAtts
%type <mySortAttList> SortAtts
...
| CREATE TABLE Name '(' CreateAtts ')' AS HEAP 
{
	ctablename = $3;
	createAtts = $5;
	cfiletype = 1;
...
| CREATE TABLE Name '(' CreateAtts ')' AS SORTED ON SortAtts 
{
	ctablename = $3;
	createAtts = $5;
	cfiletype = 2;
...
Inparams: Name INTO Name
{
	$$ = (struct InsertOperation *) malloc (sizeof (struct InsertOperation));
	$$->fname = $1;
	$$->tname = $3;
};
...
CreateAtts: Name Int
{
	$$ = (struct CreateAttList *) malloc (sizeof (struct CreateAttList));
	$$->code = INT;
	$$->name = $1;
	$$->next = NULL;
}
...
SortAtts: Name
{
	$$ = (struct SortAttList *) malloc (sizeof (struct SortAttList));
	$$->name = $1;
	$$->next = NULL;
}
| SortAtts ',' Name
