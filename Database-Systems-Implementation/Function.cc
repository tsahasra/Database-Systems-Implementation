...
void Function :: Print (struct FuncOperator *fop) {

	if (fop->right == NULL && fop->leftOperand == NULL && fop->code == '-') {
    	cout<<" - ";
    	Print (fop->leftOperator);
    	return ;
  	}
  	else if (fop->leftOperator == NULL && fop->right == NULL) {
    	if (fop->leftOperand->code == NAME || fop->leftOperand->code == INT) {
      		cout << fop->leftOperand->value;
    	}
